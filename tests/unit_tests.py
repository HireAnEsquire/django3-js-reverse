...
class AbstractJSReverseTestCase(object):
    ...
    def assertEqualJSUrlEval(self, url_call, expected_url):
        ...
        self.assertEqual(
            self.selenium.execute_script("%s return %s;" % (smart_str(response.content), url_call)), expected_url
        )
...
class JSReverseViewTestCaseMinified(AbstractJSReverseTestCase, TestCase):
    ...
    def test_view_two_url_args(self):
        self.assertEqualJSUrlEval(
            'Urls.test_two_url_args("arg_one", "arg_two")', "/test_two_url_args/arg_one-arg_two/"
        )

    def test_view_optional_url_arg(self):
        self.assertEqualJSUrlEval('Urls.test_optional_url_arg("arg_two")', "/test_optional_url_arg/2_arg_two/")
        self.assertEqualJSUrlEval(
    ...
    def test_namespaces(self):
        self.assertEqualJSUrlEval(
            'Urls["ns1:test_two_url_args"]("arg_one", "arg_two")', "/ns1/test_two_url_args/arg_one-arg_two/"
        )
        self.assertEqualJSUrlEval(
            'Urls["ns2:test_two_url_args"]("arg_one", "arg_two")', "/ns2/test_two_url_args/arg_one-arg_two/"
        )

    def test_namespaces_with_args(self):
        self.assertEqualJSUrlEval(
            'Urls["ns_arg:test_two_url_args"]("arg_one", "arg_two", "arg_three")',
            "/nsarg_one/test_two_url_args/arg_two-arg_three/",
        )

    def test_namespaces_nested(self):
        self.assertEqualJSUrlEval(
            'Urls["nestedns:ns1:test_two_url_args"]("arg_one", "arg_two")',
    ...
    def test_duplicate_name(self):
        self.assertEqualJSUrlEval('Urls.test_duplicate_name("arg_one")', "/test_duplicate_name/arg_one/")
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_name("arg_one", "arg_two")', "/test_duplicate_name/arg_one-arg_two/"
        )

    def test_duplicate_argcount(self):
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_argcount ({arg_one: "arg_one"})', "/test_duplicate_argcount/arg_one-/"
        )
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_argcount ({arg_two: "arg_two"})', "/test_duplicate_argcount/-arg_two/"
        )
        self.assertEqualJSUrlEval(
            'Urls.test_duplicate_argcount ({arg_one: "arg_one", arg_two: "arg_two"})',
